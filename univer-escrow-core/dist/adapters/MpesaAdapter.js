...
class MpesaAdapter {
    async initializePayment(amount, currency, metadata) {
        ...
        // Phase 3 spec expects STK Push Online: /mpesa/stkpush/v1/processrequest
        const businessShortCode = String(metadata?.businessShortCode ?? process.env.MPESA_SHORTCODE ?? '');
        ...
        const accountReference = String(metadata?.accountReference ?? metadata?.escrowId ?? '');
        const transactionDesc = String(metadata?.transactionDesc ?? 'Univer-Escrow collection');
        if (!businessShortCode || !passkey || !phoneNumber || !callbackUrl || !accountReference) {
            return {
                status: 'FAILED',
        ...
        const payload = {
            ...buildStkPushPayload({ ...metadata, businessShortCode, passkey, phoneNumber, callbackUrl, accountReference, transactionDesc }, amount, timestamp, buildPassword(businessShortCode, passkey, timestamp)),
        ...
        try {
            ...
            return {
                status: 'PENDING',
                transactionId: String(checkoutRequestId ?? `mpesa_${Date.now()}`),
